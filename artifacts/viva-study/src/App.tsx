...
import {
  ...
  Filter, Home as HomeIcon, KeyRound, Library, Loader2, Pencil, Plus, RotateCcw,
...
function Editor({ edit = false }: { edit?: boolean }) {
  ...
  async function generate() {
    ...
    try {
      // স্ট্রিমিং নেই — সম্পূর্ণ উত্তর তৈরি হওয়ার পরেই একবারে দেখানো হয়, যাতে
      // আংশিক/অসম্পূর্ণ টেক্সট দেখিয়ে UI ফ্লিকার না করে।
  ...
  return <div className="workspace max-w-4xl">
    ...
    <form onSubmit={submit} className="space-y-7">
      <section>
        ...
        {aiNotice && (
          <div className="ai-live-preview mt-3 flex items-center gap-2" role="status" aria-live="polite" data-testid="ai-generating-indicator">
            {generating && <Loader2 size={16} className="animate-spin shrink-0" />}
            <span className="text-sm ai-diagnostic">{aiNotice}</span>
          </div>
        )}
      </section>
