...
async function tryGemini(
  ...
  for (const model of models) {
    try {
      ...
      if (reply) {
        if (finishReason && finishReason !== 'STOP') log.push(`Gemini/${model}:finish=${finishReason}`);
        return reply;
      }
...
async function tryOpenAIChat(
  ...
  for (const model of models) {
    try {
      ...
      if (reply) {
        if (finishReason && finishReason !== 'stop') log.push(`${providerLabel}/${model}:finish=${finishReason}`);
        return reply;
      }
...
async function runWithFallback(
  ...
  // excludeAccountId দেওয়া থাকলে সেই account (সাধারণত মূল উত্তরের account)
  // candidates থেকে বাদ দেওয়া হয়, যাতে follow-up উত্তর নিশ্চিতভাবে অন্য একটি
  // AI account থেকে আসে। বাদ দেওয়ার পর যদি আর কোনো active account না থাকে,
  // তাহলে বাধ্য হয়ে বাদ দেওয়া account-টিই ব্যবহার করা হয় (fallback)।
  if (excludeAccountId !== undefined) {
    const remaining = candidates.filter(a => a.id !== excludeAccountId);
    if (remaining.length) candidates = remaining;
  }

  // preferredAccountId দেওয়া থাকলে (follow-up-দের মধ্যে ২য়, ৩য়... গুলোতে,
  // যাতে প্রথম follow-up-এ যে "২য় AI" ব্যবহার হয়েছিল সেটাই সব follow-up-এ
  // consistent ভাবে locked থাকে) সেই একই account সবসময় প্রথমে try করা হয়।
  // শুধু সেই account ব্যর্থ হলেই (key বাতিল, rate limit ইত্যাদি) অন্য active
  // account fallback হিসেবে ব্যবহার হবে।
  if (preferredAccountId !== undefined) {
    const primary = candidates.find(a => a.id === preferredAccountId);
    if (primary) {
      candidates = [primary, ...candidates.filter(a => a.id !== preferredAccountId)];
    }
  }
  ...
  // preferredAccountId বা excludeAccountId থাকলে (মানে follow-up flow চলছে)
  // provider-ভিত্তিক fixed ক্রম উপেক্ষা করে candidates-এর নিজের ক্রমেই
  // (locked/primary account সবার আগে) try করা হয়। সাধারণ মূল উত্তরের ক্ষেত্রে
  // (দুটোই undefined) PROVIDER_ORDER অনুযায়ী চলে।
  const providerOrder = (preferredAccountId !== undefined || excludeAccountId !== undefined)
    ? [...new Set(candidates.map(a => a.provider))]
    : PROVIDER_ORDER;
...
export async function generateFollowUpAnswers(
  ...
  // সব follow-up-এর জন্য একটাই "২য় AI" account লক করে রাখা হয় (মূল উত্তরের
  // account থেকে আলাদা), যাতে প্রতিটা follow-up আলাদা আলাদা random account
  // ব্যবহার না করে — প্রথম follow-up-এ যে account পাওয়া যায় সেটাকেই বাকি
  // সবগুলোতে reuse করা হয়।
  let lockedAccountId: number | undefined;
  ...
  for (const followUp of followUps.slice(0, 6)) {
    try {
      const { text, account, log } = await runWithFallback(
        ...
        lockedAccountId,
        lockedAccountId === undefined ? primaryAccount?.id : undefined,
      );
      if (lockedAccountId === undefined && account.id !== undefined) {
